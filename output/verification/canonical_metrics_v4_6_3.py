#!/usr/bin/env python3
"""
canonical_metrics_v4_6_3.py — Canonieke meetmethode GRC Kennismodel v4.6.3
=========================================================================
...
Doel
----
Reproduceerbare meting van alle structurele metrics op de v4.6.3-ontologie
(T3-sprint: 2 SKOS-predicate-substituties in m14-avg-gdpr.ttl —
2 broadMatch -> relatedMatch downgrades op compl:AVG_Art5_1f -> ctrl:5.01 en
ctrl:5.12, conform masterchat-besluit pilot-escalatie Optie C: cross-category
control<->legal-obligation is associatief, niet subsumptief). Wordt gebruikt
om de patch v4.6.3 te valideren en mutaties tegen v4.6.2 baseline te vergelijken.
...
Verschil t.o.v. v4_6_2-script
-----------------------------
- JSON-uitvoer wordt geschreven naar `output/verification/canonical_metrics_v4_6_3.json`.
- Versie-label = "v4.6.3".

Verwachte mutaties t.o.v. baseline v4.6.2
------------------------------------------
- skos_mappings_total: 1798 ongewijzigd (predicate-mutatie binnen totaal)
- skos_mappings_breakdown.broadMatch: 131 -> 129 (-2)
- skos_mappings_breakdown.relatedMatch: 192 -> 194 (+2)
- skos_mappings_breakdown.closeMatch: 1457 ongewijzigd
- skos_mappings_breakdown.exactMatch: 18 ongewijzigd
- pre-inference triples: 20950 ongewijzigd
- klassen/individuals/sameAs ongewijzigd
- m14 compl:->ctrl: SKOS-distributie: 2 closeMatch + 0 broadMatch + 29 relatedMatch
...
OUTPUT_PATH = REPO_ROOT / "output" / "verification" / "canonical_metrics_v4_6_3.json"
...
def main():
    print(f"=== canonical_metrics_v4_6_3 — {datetime.now(timezone.utc).isoformat()} ===\n")
