#!/usr/bin/env python3
"""
canonical_metrics_v4_6_2.py — Canonieke meetmethode GRC Kennismodel v4.6.2
=========================================================================
...
Doel
----
Reproduceerbare meting van alle structurele metrics op de v4.6.2-ontologie
(T2-sprint: 65 SKOS-predicate-substituties in m10-nis2-ext.ttl —
32 closeMatch -> broadMatch downgrades + 33 relatedMatch -> broadMatch
upgrades, cluster-discipline over 10 NIS2-art.21-letter-clusters). Wordt
gebruikt om de patch v4.6.2 te valideren en mutaties tegen v4.6.1 baseline
te vergelijken.
...
Verschil t.o.v. v4_6_1-script
-----------------------------
- JSON-uitvoer wordt geschreven naar `output/verification/canonical_metrics_v4_6_2.json`.
- Versie-label = "v4.6.2".

Verwachte mutaties t.o.v. baseline v4.6.1
------------------------------------------
- skos_mappings_total: 1798 ongewijzigd (predicate-mutatie binnen totaal)
- skos_mappings_breakdown.closeMatch: 1489 -> 1457 (-32)
- skos_mappings_breakdown.relatedMatch: 225 -> 192 (-33)
- skos_mappings_breakdown.broadMatch: 66 -> 131 (+65)
- pre-inference triples: 20950 ongewijzigd
- klassen/individuals/sameAs ongewijzigd
- m10 ctrl->compl SKOS-distributie: alle 118 naar broadMatch
...
OUTPUT_PATH = REPO_ROOT / "output" / "verification" / "canonical_metrics_v4_6_2.json"
...
def main():
    print(f"=== canonical_metrics_v4_6_2 — {datetime.now(timezone.utc).isoformat()} ===\n")
