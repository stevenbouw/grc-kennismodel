#!/usr/bin/env python3
"""
canonical_metrics_v4_6_4.py — Canonieke meetmethode GRC Kennismodel v4.6.4
=========================================================================
...
Doel
----
Reproduceerbare meting van alle structurele metrics op de v4.6.4-ontologie
(v4.6.4-patch: DL-conformiteits-fix — 2 rdfs:range-correcties in m21-csf.ttl,
csf:riskGovernanceDescription + csf:riskManagementDescription van xsd:string naar
rdfs:Literal, conform masterchat-besluit Optie A na HermiT-inconsistentie-vondst
op @en-getagde waarden; sluit de H38-lus). Wordt gebruikt om patch v4.6.4 te
valideren en tegen de v4.6.3 baseline te vergelijken.

Werkwijze
---------
1. Parse alle data-modules (.ttl) afzonderlijk -> per-module statistiek.
2. Merge in een graph -> globale statistiek (pre-inferentie).
...
Verschil t.o.v. v4_6_3-script
-----------------------------
- JSON-uitvoer wordt geschreven naar `output/verification/canonical_metrics_v4_6_4.json`.
- Versie-label = "v4.6.4".

Verwachte mutaties t.o.v. baseline v4.6.3
------------------------------------------
De fix vervangt alleen het OBJECT van 2 triples (xsd:string -> rdfs:Literal),
geen triple erbij/eraf. Dus ALLES identiek aan v4.6.3:
- pre-inference triples: 20950 ongewijzigd
- post-OWL-RL triples: 44907 ongewijzigd
- klassen/individuals/OP/DP/sameAs/SKOS ongewijzigd
- SKOS-mappings-totaal 1798 + breakdown ongewijzigd
Een afwijking duidt op iets anders dan een pure range-correctie -> scope-pauze.
...
OUTPUT_PATH = REPO_ROOT / "output" / "verification" / "canonical_metrics_v4_6_4.json"
...
def main():
    print(f"=== canonical_metrics_v4_6_4 — {datetime.now(timezone.utc).isoformat()} ===\n")
