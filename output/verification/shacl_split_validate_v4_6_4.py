#!/usr/bin/env python3
"""shacl_split_validate_v4_6_4.py — gesplitste SHACL-validatie v4.6.4.

Output-bestand: output/verification/shacl_results_v4_6_4.json.
...
SECTIE B (RUN 2, inference='owlrl'):
  ...
  -> verwacht 0 violations
COMBINED (inference='owlrl'):
  -> verwacht 290 false-positives op SECTIE A-shapes (bekend fenomeen baseline,
     identiek aan v4.6.2)

T3-patch v4.6.4 muteert 2 SKOS-predicates in m14-avg-gdpr.ttl (2 broadMatch
-> relatedMatch downgrades op compl:AVG_Art5_1f -> ctrl:5.01 en ctrl:5.12).
Geen van de actieve shapes valideert direct op compl:<->ctrl:-mapping-distributie
(SHACL-blinde vlek op deze cluster, H39-trigger; bevestigd in T3 pre-sprint-
inventarisatie §8.4). Verwachting: identieke violation-counts t.o.v. v4.6.2
(0 / 0 / 290).
"""
import sys
import json
...
OUTPUT_PATH = REPO_ROOT / "output" / "verification" / "shacl_results_v4_6_4.json"
...
print(f"COMBINED (owlrl):      {n_c:>4} violations (verwacht 290, Δ vs v4.6.2 = {n_c - 290})")
