#!/usr/bin/env python3
"""
canonical_metrics_v4.6.0.py — Canonieke meetmethode GRC Kennismodel v4.6.0
=========================================================================

Doel
----
Reproduceerbare, eenduidige meting van alle structurele metrics op
de v4.6.0-ontologie. Vervangt ad-hoc tellingen uit eerdere
opleveringsrapporten en lost bevinding D (meetmethode-verschil) op.

Werkwijze
---------
1. Parse alle 21 data-modules (incl. nieuwe m21-csf.ttl) (.ttl) afzonderlijk → per-module statistiek.
...
5. Tweetaligheid (D6), TBox-vs-ABox vulling, integriteitschecks.

Output
------
- JSON-bundel `canonical_metrics_v4.6.0.json` (machine-leesbaar)
- Tekstuele samenvatting op stdout
...
def is_grc_uri(node) -> bool:
    return isinstance(node, URIRef) and str(node).startswith(GRC_BASE)
...
def per_module_metrics(workdir: Path) -> dict[str, dict[str, Any]]:
    """Parse elke .ttl los; tel basismetrics per bestand."""
...
def global_metrics(g: Graph, label: str) -> dict[str, Any]:
    ...
    # Labels / comments per taal globaal
    lbl_lang = Counter()
    cmt_lang = Counter()
...
def apply_owl_rl(g: Graph) -> None:
    """OWL RL inferentie volgens projectconventie."""
...
# ── 3. Per-namespace metrics op gemergde graph ─────────────────────────
...
def per_namespace_metrics(g: Graph) -> dict[str, dict[str, int]]:
    ...
    # Subjects en objects per namespace
...
# ── 4. SKOS-mapping per framework-paar ─────────────────────────────────

def skos_pair_metrics(g: Graph) -> dict[str, dict[str, int]]:
    """Telt SKOS-mappings per (subject_namespace, object_namespace, mappingType)."""
...
# ── 5. TBox/ABox vulling per domein ────────────────────────────────────

def tbox_abox_per_namespace(g: Graph) -> dict[str, dict[str, Any]]:
    """Voor elke namespace: hoeveel klassen vs hoeveel daarvan instances hebben."""
    out: dict[str, dict[str, Any]] = {}
    for pfx, ns in NAMESPACES.items():
        # Alle gedeclareerde klassen in deze ns
        classes = {s for s in g.subjects(RDF.type, OWL.Class)
                   if isinstance(s, URIRef) and str(s).startswith(ns)}
        # Daarvan: hoeveel hebben minimaal 1 instance?
        cls_with_inst = 0
        for c in classes:
            if any(g.subjects(RDF.type, c)):
                cls_with_inst += 1

        # Properties in deze ns
...
# ── 6. Integriteitschecks ──────────────────────────────────────────────

def integrity_checks(g: Graph) -> dict[str, Any]:
    findings: dict[str, Any] = {}

    # 6.1 Individuals zonder rdf:type
    ...
    # 6.2 ObjectProperties / DatatypeProperties zonder domain of range
    op_no_dom, op_no_rng = [], []
    ...
    # 6.3 Dangling references: GRC-IRIs als object gebruikt zonder eigen triples als subject
    ...
    # 6.4 Dubbele rdf:type owl:Class declaraties
    ...
    # 6.5 Namespace-leakage: GRC-prefix maar buiten de 10 bekende namespaces
    leaked = set()
    ...
    # 6.7 owl:sameAs samenstelling (D5 + D11)
...
def d_decision_conformance(g: Graph) -> dict[str, Any]:
    ...
    # D7: BIO 2.0 strikt 2 klassen — geen ctrl:BIO2-overblijfselen
...
def main():
    print(f"=== canonical_metrics_v4.6.0 — {datetime.now(timezone.utc).isoformat()} ===\n")
    ...
    out_path = Path("/home/claude/canonical_metrics_v4.6.0.json")
    out_path.write_text(json.dumps(output, indent=2, ensure_ascii=False))
    print(f"\n✓ JSON-bundel: {out_path}")
    print(f"\n--- KERNSAMENVATTING ---")
